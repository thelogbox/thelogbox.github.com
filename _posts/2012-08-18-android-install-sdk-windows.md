...
**GET THE ZIPPED FILE**. Unzip the file to a user directory (so that you will not need elevated privileges to use the executable files). The user directories are usually inside <code class="codeblock">C:\User\yourname\</code>. Try to remember the full path of the directory where you unzipped the Android SDK, this will be the **ANDROID_HOME**

**GET A TERMINAL WINDOW**, launch *cmd.exe*. Go to the ANDROID-HOME/tools directory, run the <code class="codeblock">android</code> executable. This will launch the Android SDK Manager. 
